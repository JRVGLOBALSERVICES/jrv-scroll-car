...
// ── Video + Content in One Block ──
function HeroAndContent() {
  const [ended, setEnded] = useState(false);
  const handleProgress = useCallback((p: number) => { if (p >= 1 && !ended) setEnded(true); }, [ended]);
    ...
    <>
      ...
      <div
        style={{
          ...
          zIndex: ended ? 1 : 10,
          background: ended ? "#fff" : "#111118",
        }}
      >
        {/* Video section — exactly one viewport */}
        <div className="relative h-screen w-full overflow-hidden">
          ...
          <motion.div animate={{ opacity: ended ? 0 : 1 }}
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 max-w-sm">
              ...
              <a href="#fleet" className="border border-white/15 text-white/70 text-center font-semibold px-7 py-3 rounded-xl text-sm hover:bg-white/5 transition-all">Explore</a>
            </div>
          </motion.div>
        </div>

        {/* Content sections — inside the block, so they DON'T move until video ends */}
        <div className="relative">
          {/* Stats */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={ended ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, ease: [0.23, 1, 0.32, 1] }} className="bg-white border-b border-gray-100 py-6 md:py-8">
          ...
          <section id="fleet" className="py-16 md:py-20 bg-white">
            <div className="max-w-5xl mx-auto px-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 md:gap-4">
                {cars.map(car => (
                  <motion.div key={car.n} initial={{ opacity: 0, y: 20 }} animate={ended ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, ease: [0.23, 1, 0.32, 1] }} className="bg-white border border-gray-200 rounded-xl p-4 md:p-5 hover:border-[#FF4500]/30 hover:shadow-lg transition-all">
          ...
          <section className="py-16 md:py-20 bg-[#FFF8F0] border-t border-gray-100">
            <div className="max-w-5xl mx-auto px-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 md:gap-4">
                {features.map(f => (
                  <motion.div key={f.t} initial={{ opacity: 0, y: 15 }} animate={ended ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, ease: [0.23, 1, 0.32, 1] }} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-[#FF4500]/20 hover:shadow-md transition-all">
